...
void Camera::startTimers()
{
  ...
  if (runStatusTimer_ && statusIntervalSeconds_ > 0) {
    if (statusTimer_) {
      statusTimer_->cancel();
    }
    statusTimer_ = rclcpp::create_timer(
      node_base_interface_, node_timers_interface_, clock(),
      rclcpp::Duration::from_seconds(statusIntervalSeconds_),
...
void Camera::readParameters()
{
  ...
  statusIntervalSeconds_ = safe_declare<double>(prefix_ + "status_interval_seconds", 5.0);
